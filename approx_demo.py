...
# pairs = dict(all_pairs_dijkstra(graph))
# pairs[node1][0][node2] -> weight
# pairs[node1][1][node2] -> path
...
def approx_steiner(graph, terms):
    res=[]
    pairs = dict(nx.all_pairs_dijkstra(graph))

    print(pairs[1])
    # couverture transitive du sous-graphe des terms
    complete_graph = nx.complete_graph(terms)
    for i, j in it.combinations(terms, 2) :
        dict_dist,_ = pairs[i]
        complete_graph[i][j]["weight"] = dict_dist[j]


    min_span = nx.minimum_spanning_tree(complete_graph)

    print_graph(graph)
    print_graph(complete_graph)
    print_graph(min_span)
    print(complete_graph.edges)
    print(min_span.edges)

    for i, j in min_span.edges :
        _, chemins = pairs[i]
        chemin = chemins[j]
        # print(chemin)
        for i in range(len(chemin) - 1) :
            edge = [chemin[i], chemin[i+1]]
            if edge not in res : res.append(edge)
    return res
